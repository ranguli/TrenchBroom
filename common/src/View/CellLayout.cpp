...
void CellLayout::addGroup(
  std::any groupItem, const float titleHeight, const bool collapsed)
{
  if (!m_valid)
  {
    validate();
  ...
  const auto& newGroup = m_groups.emplace_back(
    std::move(groupItem),
    m_outerMargin,
    y,
    m_cellMargin,
    ...
    m_maxCellHeight,
    collapsed);
  m_height += newGroup.bounds().height;
}
...
void CellLayout::validate()
{
  ...
  if (!m_groups.empty())
  {
    auto copy = std::move(m_groups);
    m_groups.clear();

    for (const auto& group : copy)
    {
      addGroup(group.item(), group.titleBounds().height, group.collapsed());
