...
TEST_CASE("DiskFileSystemTest.constructor")
{
...
TEST_CASE("DiskFileSystemTest.makeAbsolute")
{
  const auto env = makeTestEnvironment();
...
TEST_CASE("DiskFileSystemTest.pathInfo")
{
  const auto env = makeTestEnvironment();
  const auto fs = DiskFileSystem{env.dir()};
...
TEST_CASE("DiskFileSystemTest.directoryContents")
{
  const auto env = makeTestEnvironment();
  const auto fs = DiskFileSystem{env.dir()};
