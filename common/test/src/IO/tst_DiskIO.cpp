...
TEST_CASE("DiskIO::pathInfo")
{
  const auto env = makeTestEnvironment();
}

TEST_CASE("DiskIO::directoryExists")
{
  const auto env = makeTestEnvironment();
  ...
  CHECK(Disk::pathInfo(env.dir() + Path{"ANOTHERDIR"}) == PathInfo::Directory);
  CHECK(Disk::pathInfo(env.dir() + Path{"anotherDir/subDirTest"}) == PathInfo::Directory);
...
TEST_CASE("DiskIO::directoryContents")
{
  const auto env = makeTestEnvironment();
